...
model = load_model('cpu_model-log.h5')
...
for i in range(len(expectation)):
    expectation[i] = np.e**expectation[i]
    predictions[i] = np.e**predictions[i]
...
pyplot.savefig('pred9-model-3.png')
pyplot.show()
